...
void print_char(char c, int x, int y, color color, int size){
    ...
    for(int yy = 0; yy < FONT_WIDTH*size; yy++){
        for(int xx = 0; xx < FONT_HEIGHT*size; xx++){

            if(glyph[yy/size] & (1<<xx/size)){
...
void print_string(char *s, int x, int y, color color, int size){
    ...
    while(s[index] != 0){
        print_char(s[index++], x, y, color, size);
        x+=FONT_WIDTH*size;

        if(x >= SCREEN_WIDTH-FONT_WIDTH*size){
            x=0;
            y+=FONT_HEIGHT*size; 
        }
    }
